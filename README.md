
## Tim

| Anggota | Peran | Branch |
|---|---|---|
| Eno | Project Manager | `pm/project-management` |
| Rapid | Backend Developer | `backend/rapid-auth-user-blog` |
| Angel | Backend Developer | `backend/angel-exchange-admin` |
| Diana | Frontend Developer | `frontend/diana-public-user` |
| Rifa | Frontend Developer | `frontend/rifa-admin-exchange` |

## Urutan integrasi

1. Baseline Eno
2. Backend autentikasi Rapid
3. Backend transaksi dan admin Angel
4. Backend user dan blog Rapid
5. Frontend publik dan user Diana
6. Frontend admin dan penukaran Rifa
